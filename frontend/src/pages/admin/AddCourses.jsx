...
const editCourseHandler = async () => {
  ...
  // ✅ always define updatedCourse first
  const updatedCourse = result.data;

  let updatedCourses = courseData.map(c =>
    c._id === courseId ? updatedCourse : c
  );

  // ✅ if the course doesn't already exist, push it
  if (!courseData.some(c => c._id === courseId)) {
    updatedCourses.push(updatedCourse);
  }
...
function AddCourses() {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 mt-10 bg-white rounded-lg shadow-md">
      {/* Top Bar 
      <div className="flex items-center justify-between mb-6 relative flex-col md:flex-row gap-4">
        <FaArrowLeftLong
          className="absolute top-[-20px] md:top-[20%] left-0 md:left-[2%] w-6 h-6 cursor-pointer"
      ...
      <div className="bg-gray-50 p-6 rounded-md">
        ...
        <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="relative w-[300px] h-[170px] mb-4">
            <img
              src={frontendImage}
              alt="thumbnail"
          ...
          <div className="flex items-center gap-4">
            <button
              className="bg-gray-300 hover:bg-red-200 text-black px-4 py-2 rounded-md"
              onClick={() => navigate("/courses")}
            >
              Cancel
            </button>
            <button
              className="bg-black text-white px-7 py-2 rounded-md hover:bg-gray-700 flex items-center justify-center"
              disabled={loading}
